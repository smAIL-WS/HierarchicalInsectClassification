# Plot the F1-score performance as a function of the number of images per class
import pandas as pd
import numpy as np
...
X_COLUMN = 6              # independent variable
Y_COLUMN = 11              # dependent variable
GROUP_COLUMN = 0            # hierarchy level

USE_POINT_LABELS = True  # set True if you want labels drawn

FIGURE_TITLE = None # "F1-score vs. Number of images per class"
X_AXIS_LABEL = "Number of images per class (log. scale)"
...
# label_name: (horizontal_align, vertical_align, dx, dy)
    "Brachycera": ("right", "top",    -2, -1),   # bottom-left of point
    "Tephritidae": ("right", "top",    -1, -1),
    "Hymenoptera": ("left", "bottom",    1, 1),
    "Nematocera": ("right", "bottom",    -2, -1),
    "Formicidae": ("right", "top",    -1, -1),
    "Orthoptera": ("right", "bottom",    -1, 0),
    "Asiloidea": ("left", "bottom",    1, 1),
    "Lauxanioidea": ("right", "top",    -1, -1),
    "Heterocera": ("left", "top",    1, -1),
    "Oestroidea": ("right", "bottom",    -1, -1),
    "Syrphidae": ("left", "top",    0, -1),
    "Aculeata": ("left", "bottom",    1, 1),
    "Apocrita": ("left", "bottom", 1, 0),
    "Mecoptera": ("right", "bottom", -1, -1)
}
...
plt.savefig(
    OUTPUT_DIR / "F1_performance_num_images.svg",
    format="svg",
    bbox_inches="tight"
)

plt.savefig(
    OUTPUT_DIR / "F1_performance_num_images.png",
    dpi=300,
    bbox_inches="tight"
)
